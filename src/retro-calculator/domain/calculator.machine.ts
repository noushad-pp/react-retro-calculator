...
const not =
  ...
  (...args: any) =>
    !fn(...args);
const isZero = (_context: any, event: any) => event.key === 0;
const isNotZero = not(isZero);
const isMinus = (_context: any, event: any) => event.operator === '-';
const isNotMinus = not(isMinus);
const divideByZero = (context: any, _event: any) => {
  return (!context.operand2 || context.operand2 === '0.') && context.operator === '/';
};
...
const calculatorMachine = Machine<Context>(
  {
    id: 'calcMachine',
    context: {
      display: '0.',
  ...
  },
  {
    ...
    actions: {
      ...
      setNegativeReadoutNum: assign({
        display: (_context, event) => `-${event.key}.`,
      }),
